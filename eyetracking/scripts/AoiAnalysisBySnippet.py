...
def run_calculations(preprocessed_data, group, scrambled, results_for_anova):
    ...
    for i, snippet in enumerate(config.SNIPPETS_UNIQUE):
        ...
        overall_fixations = len(fixations['1440'])
        ...
        for resolution in resolutions:
            ...
            for fixation in fixations[resolution]:
                ...
                for nr_line, aoi_line in enumerate(AOIs['lines']):
                    if aoi_line['x_left'] <= x < (aoi_line['x_right'] + HORIZONTAL_AOI_DEVIATION_THRESHOLD):
                        if aoi_line['y_bottom'] <= y < aoi_line['y_top']:
                            aoi_line_hit = nr_line + 1
                            break

                for nr_types, aoi_types in enumerate(AOIs['types']):
                    if aoi_types['x_left'] <= x < aoi_types['x_right']:
                        if aoi_types['y_bottom'] <= y < aoi_types['y_top']:
                ...
                for nr_blocks, aoi_blocks in enumerate(AOIs['blocks']):
                    if aoi_blocks['x_left'] <= x < aoi_blocks['x_right']:
                        if aoi_blocks['y_bottom'] <= y < aoi_blocks['y_top']:
                            aoi_blocks_hit = nr_blocks + 1
                            break

                # if AOIs['answer']['x_left'] <= x < AOIs['answer']['x_right']:
                #     if AOIs['answer']['y_bottom'] <= y < AOIs['answer']['y_top']:
                #         is_answer_hit = True
        ...
        if overall_fixations != 0:
            for participant in matched_fixations_by_participant:
                ...
                results_for_anova = results_for_anova._append(result_for_participant, ignore_index=True)

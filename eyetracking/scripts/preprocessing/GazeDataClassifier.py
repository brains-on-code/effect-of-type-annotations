...
class GazeDataClassifier:
    ...
    def compute_fixations_saccades(participant_df):
        ...
        for i, event in enumerate(events):
            ...
            if current_event['Type'] == event and current_event['Snippet'] == participant_df['gaze_data'].iloc[i]['Snippet']:
                current_event['Data'] = current_event['Data']._append(participant_df['gaze_data'].iloc[[i]], ignore_index=True) # TODO: check if changed append to _append
    ...
    def reduce_gaze_dataframe(participant_dict):
        participant_dict['gaze_data'] = participant_dict['gaze_data'].drop(participant_dict['gaze_data'].columns.difference(['Time', 'GazePosX', 'GazePosY', 'Velocity', 'Snippet', 'ExperimentTime', 'Gaze_Event', 'TrialNumber']), axis=1)
        participant_dict['gaze_data'] = participant_dict['gaze_data'].round({'GazePosX': 2, 'GazePosY':2, 'Velocity': 2})

        # participant_dict['gaze_data']['meaningful'] = participant_dict['gaze_data']['Snippet'].apply(lambda x: True if x[-2] == 'M' else False)
        # participant_dict['gaze_data']['typeAnnotation'] = participant_dict['gaze_data']['Snippet'].apply(lambda x: True if x[-1] == 'T' else False)
        # participant_dict['gaze_data']['Snippet'] = participant_dict['gaze_data']['Snippet'].apply(lambda x: x[:-2] if x[-2] == 'M' or x[-2] == 'L' else x)

class GazeDataCleaner:
    ...
    def drop_unnecessary_columns(gdf):
        # return gdf.drop('Indentation', axis=1) # we don't have this column anymore
        return gdf
    ...
    def drop_fixation_cross_rows(gdf):
        ...
        original_size = gdf.shape[0]
        gdf = gdf[~gdf['Snippet'].str.contains("WarmUp")]
        reduced_size = gdf.shape[0]
        gdf.reset_index(inplace=True)

        print('Dropped some eye-gaze data from WarmUp condition: ', str(original_size - reduced_size))

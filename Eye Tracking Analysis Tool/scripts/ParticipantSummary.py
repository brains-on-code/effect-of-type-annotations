...
def preprocess_raw_data(group):
    ...
    general_data = pd.read_csv(path.join('studies', config.CURRENT_STUDY, 'GeneralInfo_' + group + '.csv'),
     delimiter=';')
...
def load_preprocessed(group='AllParticipants'):
    print('\n### Preprocessing Pipeline from Raw Data')
    if group == 'AllParticipants':
        print('Reading novice data...')
        with open(path.join(config.PATH_DATA_RAW, "PreprocessedDataAllParticipants.pkl"), 'rb') as input:
...
def run_analysis_pipeline(participant_dfs):
    ...
    for participant_df in participant_dfs:
        ...
        participant_df = ParticipantFixationSaccade.participant_fixation_saccades_TA(participant_df)
